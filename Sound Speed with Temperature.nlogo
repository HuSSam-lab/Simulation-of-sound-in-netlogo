...
to setup
  ...
  set-default-shape sounds "target"
  set-default-shape listeners "person"

  set heard 0
  set speed round(331 + 331 * (temp * 0.00181))
  set tempOld temp

  set speed-of-sound 331
  set initial-wave-amplitude 20 ; how loud is the wave when first emitted?
  set wave-interval 3          ; how often does the sound emit a wave?
  ...
  create-sounds 1 [
    set heading 90
    set ycor 10 + min-pycor
    set xcor 20 + min-pxcor
    set size 4
    set color white
  ]
...
to go
  if not (temp = tempOld) [
    ...
    [ask patches[set pcolor red - (round ((5 / (temp + 5)) * 3))]]
  ]
  ...
  if ticks mod wave-interval = 0 [ ask sounds [ emit-wave ] ] ;; emit the sound wave
                                                              ;; move waves
  ask wave-components [
    if not can-move? (0.5 + (0.01 * temp)) [ die ]
    fd (0.5 + (0.01 * temp))
    set amplitude amplitude - (0.5 + (0.01 * temp))
    set color scale-color blue amplitude 0 initial-wave-amplitude
    if amplitude < (0.5 + (0.01 * temp)) [ die ]
  ]

  ;; listen and plot
  ask listeners [
    let amp amplitude-here wave-ids-heard
    set-current-plot "Signal"
    if amp > 0 and heard = 0 [
      plot heard
      set heard 1
      plotxy ticks 1
      set time_arrived ticks
      output-print ""
      output-print "Temperature changed!"
      output-print "Current Temperature: "
      output-print temp
      output-print "Speed Temperature: "
      output-print speed
      output-print "Time Arrived: "
      output-print time_arrived
      output-print ""
      sound:play-drum "ACOUSTIC SNARE" 100
      stop
    ]
    set wave-ids-heard
    remove-duplicates
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot speed"
...
1444
346
المسألة العامة الأولى              \nإذا كانت سرعة انتشار الصوت في الهواء       \n         \n                 v = 331 m.s-1\n\n                                   t = 0c          بدرجة\n\nاحسب سرعة انتشار الصوت                      \n\nبالدرجة        t = 27c                                 
20
0.0
0
...
1498
552
----------------------------------------------------------------------------\nالحل:                                             \n\nv1 / v2 = sqr(T1) / sqr(T2)\n\n=> 331 / v2 = sqr(oldTemp + 273) / sqr(temp + 273) = 1.098\n\n=> v2 = 347 m.s-1\n
18
0.0
1
...
1506
78
الفكرة العامة للمشروع هي عبارة عن قياس سرعة انتشار الصوت في درجات الحرارة المختلفة                             
18
0.0
1
